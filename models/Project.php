...
class Project extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['title', 'description', 'bond', 'period', 'place'], 'required'],
            [['description', 'bond', 'co_hosts', 'partners'], 'string'],
            [['organizational_unit_id', 'created_at', 'updated_at'], 'integer'],
            [['title'], 'string', 'max' => 100],
            [['period', 'place'], 'string', 'max' => 255],
            [['title', 'description', 'bond', 'period', 'place', 'co_hosts', 'partners'], 'filter', 'filter'=>function($value) {return trim(strip_tags($value));}],
    ...
    private function _runWorkflowChecks($event)
    {
        ...
        switch ($event->getEndStatus()->getId()) {
            case 'ProjectWorkflow/submitted':
                if ($this->bond=='...') {
                    $this->workflowError = 'New projects must have their bonds declared.';
    ...
    private function _runWorkflowRoutines($event)
    {
        ...
        if ($event->getEndStatus()->getId() == 'ProjectWorkflow/draft') {
            if (in_array($event->getStartStatus()->getId(), ['ProjectWorkflow/submitted', 'ProjectWorkflow/approved'])) {
                $this->_createCommentAfterResettingToDraft();
            }
    ...
    {
        return $this->_createAutogeneratedComment(Yii::t('app', 'Submitted planned expenses') . "\n\n" . $this->getFrozenPlannedExpensesAsText());
    }

    private function _createCommentAfterResettingToDraft()
    {
        return $this->_createAutogeneratedComment(Yii::t('app', 'Reset to draft after submission'));
    }
    
    private function _createAutogeneratedComment($text)
    {
        $comment = new ProjectComment();
        $comment->user_id = Yii::$app->user->identity->id;
        $comment->project_id = $this->id;
        $comment->comment = Yii::t('app', '[Autogenerated comment]') . ' - '. $text;
        $comment->save();
        return $comment->id;
    }
    ...
    {
        return 
            $this->getAllRelatedExpenses()->count()
            ==
            $this->getAllRelatedExpenses()->withTransactionStatus('TransactionWorkflow/recorded')->count()+
            $this->getAllRelatedExpenses()->withTransactionStatus('TransactionWorkflow/reimbursed')->count() +
            $this->getAllRelatedExpenses()->withTransactionStatus('TransactionWorkflow/extra')->count()
        ;
    }
