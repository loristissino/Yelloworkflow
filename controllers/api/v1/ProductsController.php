<?php

namespace app\controllers\api\v1;

use yii\data\ActiveDataProvider;
use app\components\RestController;
use app\components\CookieAuth;
use app\models\ProductResource;
use app\models\OrganizationalUnit;

class ProductsController extends RestController
{
	public $modelClass = '\app\models\ProductResource';
	
    public function behaviors()
    {
        $behaviors = parent::behaviors();
        $behaviors['authenticator'] = [
            'class' => CookieAuth::className(),
        ];
        unset($behaviors['access']);
        
        return $behaviors;
    }    
    
	public function prepareDataProvider()
	{
        // $ou = OrganizationalUnit::findOne(\Yii::$app->session->get('organizational_unit_id'));
        $identity = \Yii::$app->user->identity;
        
        if ($identity === null) {
            throw new \yii\web\UnauthorizedHttpException('Not authenticated');
        }
        
        $ou = \app\models\OrganizationalUnitResource::findOne($identity->getCurrentChosenOrganizationalUnit()->id);
        
		$query = ProductResource::find()->salableByOrganizationalUnit($ou)->orderBy(['rank'=> SORT_ASC, 'description' => SORT_ASC]);

		$provider = new ActiveDataProvider([
			'query' => $query,
			'pagination' => [
				'pageSize' => 1000,
			],
			'sort' => [
				'defaultOrder' => [
					'rank' => SORT_ASC, 
				]
			],
		]);
        
        // die(\Yii::$app->user->identity);
        // die(\Yii::$app->session->get('organizational_unit_id'));
		
		return $provider;
	}
}
