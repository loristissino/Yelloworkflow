...
class ShorteningsController extends CController
{
     ...
     * @return mixed
     */
    public function actionCreate($multiline=false)
    {
        $model = new Shortener($multiline);

        if ($model->load(Yii::$app->request->post())) {
            if ($model->save()) {
                if ($model->multiline) {
                    \app\components\LogHelper::log(sprintf('created multiple shorteners: %d', sizeof($model->response)), $model);
                    Yii::$app->session['shortener_output'] = $model->response;
                    return $this->redirect('multiline');
                }
    ...
    public function actionMultiline()
    {
